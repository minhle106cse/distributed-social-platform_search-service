...
export class IndexKnowledgeHandler implements IIntegrationEventHandler<KnowledgePublishedPayload> {
  ...
  // Safe under redelivery: both writes are idempotent by itemId — pgvector
  // replaceForItem (delete+insert) and ES indexItem (upsert by id). Re-applying
  // a redelivered event is a no-op.

  constructor(
    @Inject(EMBEDDING_SERVICE) private readonly embedding: IEmbeddingService,
    @Inject(TX_RUNNER) private readonly txRunner: ITxRunner<SearchTxScope>,
  ...
  async handle(event: CloudEvent<KnowledgePublishedPayload>): Promise<void> {
    ...
    if (chunks.length === 0) {
      ...
      await this.txRunner.run((tx) => tx.chunks.replaceForItem(itemId, []))
      this.logger.info(
    ...
    await this.txRunner.run((tx) => tx.chunks.replaceForItem(itemId, rows))
